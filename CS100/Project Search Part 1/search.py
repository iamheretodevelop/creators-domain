...
def article_count(count, titles):
  title_list = []
  if count > len(titles) or len(titles) == 0:
    return title_list
...
#   titles - lmist of article titles to search through
#
...
def random_article(index, titles):
  if index > len(titles) - 1 or index < -len(titles):
    return ""
  return titles[index]
...
# Prints out articles based on searched keyword and 
def display_result():
